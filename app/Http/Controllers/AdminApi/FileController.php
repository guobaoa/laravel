...
//		Log::info($info);
    ...
    public function postKeditFile(Request $request, \getID3 $getId3)
    {
        ...
        if (!$request->hasFile($this->filed_name) && !$request->hasFile($this->filed_img_name)) {
            return response('必须提供 file 字段数据。', 400);
        }

        $filed_name = $this->filed_name;
        if ($request->hasFile($this->filed_img_name)) {
            $filed_name = $this->filed_img_name;
        }
        ...
        $userfile->save();
        // 返回结果。
//		Queue::push(new QiniuJob($userfile->id));
