...
class CasesController extends Controller
{
    ...
    public function getList(Request $request)
    {
        ...
        $data = $data
            ...
            ->select('cases.*', 'categorys.initials_name')
    ...
    {
        $id = $request->input('id');
        $data = Cases::join('categorys', 'cases.category_id', '=', 'categorys.id')
            ->select('cases.*', 'categorys.initials_name')
            ->find($id);
        ...
        $lastId = Cases::where('id','<',$id)->max('id');
        $list['lastpage'] = Cases::select('id','title')->find($lastId);
    
        $nextId = Cases::where('id','>',$id)->min('id');
        $list['nextpage'] = Cases::select('id','title')->find($nextId);
    
        $idArr = [$lastId,$id,$nextId];
        $listData = Cases::whereNotIn('id',$idArr)->limit(3)->get();
